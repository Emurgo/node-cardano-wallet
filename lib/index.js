...
var RandomAddressChecker = {
  newChecker: function (xprv) {
    return handleResultString(
      rust.random_checker_new_checker(JSON.stringify(xprv.toString('hex')))
    );
  },
...
var Wallet = {
  ...
  checkAddress: function (address) { // Buffer
    return handleResultString(
      rust.wallet_check_address(JSON.stringify(address.toString('hex')))
    )
  },
