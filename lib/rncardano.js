...
var PasswordProtect = {
  ...
  decryptWithPassword: function (password, data) {
    return Promise.resolve().then(function () {
      return node.PasswordProtect.decryptWithPassword(password, Buffer.from(data, 'hex')).toString('hex');
    });
  }
};
...
var HdWallet = {
  fromEnhancedEntropy: function (entropy, password) {
    return Promise.resolve().then(function() {
      return node.HdWallet.fromEnhancedEntropy(Buffer.from(entropy, 'hex'), password).toString('hex');
    });
  },
...
var Wallet = {
  ...
  checkAddress: function (address) {
    return Promise.resolve().then(function() {
      return node.Wallet.checkAddress(Buffer.from(address, 'hex'));
    });
  },
